...
const FONT_REG    = path.join(__dirnameR, '..', 'fonts', 'DejaVuSans.ttf');
const FONT_BLD    = path.join(__dirnameR, '..', 'fonts', 'DejaVuSans-Bold.ttf');
...
// ── GET /api/suscriptores/movil/rutinas/:id/pdf?token=<jwt> ──────────────────
// Se abre en el navegador del dispositivo: el token va en query param.

router.get('/movil/rutinas/:id/pdf', async (req, res) => {
  try {
    ...
    try {
      ...
      if (payload.rol !== 'suscriptor') return res.status(403).send('Acceso exclusivo para suscriptores');
    } catch {
    ...
    const [[rutina]] = await db.query(
      `SELECT r.id_rutina, r.notas_pdf,
              DATE_FORMAT(r.creado_en, '%Y-%m-%dT%H:%i:%s.000Z') AS creado_en,
              COALESCE(CONCAT(p.nombres, ' ', p.apellido_paterno), 'Entrenador') AS entrenador,
              COALESCE(sus.nombres, '') AS suscriptor_nombre
       FROM rutinas r
       LEFT JOIN personal    p   ON p.id_personal  = r.id_entrenador
    ...
    const [ejercicios] = await db.query(
      `SELECT re.orden, re.series, re.repeticiones, re.descanso_seg,
              re.peso_kg, re.descripcion_tecnica, re.nombre_bloque,
    ...
    console.error('[GET /suscriptores/movil/rutinas/:id/pdf]', err);
